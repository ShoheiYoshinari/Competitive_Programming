...
void fortuneTellingAdjustment(vector<vector<int>> &groups, const vector<City>& cities, int L, int m, int &queriesUsed, int Q) {
    rep(g, m) {
        if (queriesUsed >= Q) break;
        if (groups[g].size() < 2) continue;

        int querySize = min((int)groups[g].size(), L);
        vector<pair<double, int>> distCity;
        ...
        double cx = sumx / groups[g].size(), cy = sumy / groups[g].size();

        for (int id : groups[g]) {
            double d = dist2(cities[id].cx, cities[id].cy, cx, cy);
            distCity.push_back({d, id});
        }
        sort(all(distCity));

        vector<int> queryCities;
        rep(i, querySize) queryCities.push_back(distCity[i].second);

        cout << "? " << querySize;
        for (int id : queryCities) cout << " " << id;
        cout << "\n" << flush;
        queriesUsed++;
...
int main(){
    ...
    // 占いクエリ
    int queriesUsed = 0;
    fortuneTellingAdjustment(groups, cities, L, m, queriesUsed, q);
