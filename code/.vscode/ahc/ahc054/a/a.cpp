...
void place_vertical_walls_trent() {
    ...
    for(int col : target_cols) {
        ...
        int max_to_place = min((int)candidates.size(), n/3);
        rep(k, max_to_place){
...
void stick_down_walls_trent(){
    random_device rd;
    mt19937 g(rd());

    vector<pair<int, int>> base_sticks;
    for(int i = 0; i < n; i += 2){
        for(int j = 0; j < n; j += 2){
            if(can_place(i, j)){
                b[i][j] = 'T';
    ...
    for(auto [x,y] : base_sticks){
        ...
        while(!q.empty()){
            ...
            int ni = i+directions[cnt%4].first, nj = j+directions[cnt%4].second;
            if(!can_place(ni,nj)) continue;
